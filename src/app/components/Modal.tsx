...
export default function ResearchModal() {
  ...
  const handleStageBlur = (index: number) => {
    confirmStage(index);
  };
  ...
  useEffect(() => {
    ...
    if (last.question.trim() !== "") {
      setQuestions((prev) => [...prev, { id: Date.now(), question: "", stage: "", confirmed: false, loading: false, error: null, disabled: true, manuallyEditing: false }]);
    }
  }, [questions]);
          ...
          >
            <div className="min-h-0 flex-1 overflow-y-auto">
              <div className="mb-3 flex items-center justify-between border-b border-gray-200 px-4 py-4">
                <input
                  className="peer block w-full rounded-md border-none p-0 text-xl font-semibold text-gray-800 placeholder:text-gray-400 placeholder:font-normal focus:outline-none"
              ...
              <div className="px-4">
                ...
                <div className="space-y-2 pt-2">
                  {questions.map((q, index) => (
                    <div
                      key={q.id}
                      className="relative grid grid-cols-10 items-start gap-2 border-b border-gray-100 py-1 text-sm min-h-[50px]"
                    >
                      <textarea
                        ...
                        rows={1}
                      />
                      <div className="col-span-3 h-[34px] relative">
                        {!q.question.trim() ? null : q.confirmed ? (
                          <div className="inline-flex h-full items-center gap-1 rounded-md bg-green-100 border border-green-700 text-green-700 px-2 text-xs">
                            {q.stage}
                            <button
                              type="button"
                          ...
                          <input
                            ...
                            onBlur={() => handleStageBlur(index)}
                            disabled={q.disabled}
                            className="w-full h-full border border-gray-200 rounded-md px-2 py-1 text-sm text-gray-900 focus:outline-none disabled:bg-gray-100"
                          />
                        ) : !q.loading ? (
                          <button
                            type="button"
                        ...
                        {q.error && (
                          <div className="absolute mt-1 text-xs text-red-500 left-0 w-full">
                            {q.error}
                          </div>
                        )}
                      </div>
            ...
            <div className="shrink-0 border-t border-gray-200 bg-white px-4 py-4 flex justify-between gap-4">
              ...
              <button
                type="button"
                className="inline-flex items-center justify-center rounded-md text-white px-4 py-2 text-sm bg-[#1E56B8] hover:bg-[#163F8C] focus:outline focus:outline-4 focus:outline-[rgba(207,_219,_231,_0.35)]"
              >
                Next: Add interviews
              </button>
            </div>
